...
class TweetApiTests(TestCase):
    ...
    def test_create_with_files(self):
        # hanyuan test: no file in upload
        response = self.hanyuan_client.post(TWEET_CREATE_API, {
            'content': 'a selfie',
            'files': [],
        })
        ...
        # upload one file
        # content needs to be a type bytes, use str.encode to convert
        ...
        # test upload for multiple files
        ...
        # make sure addr of photo is included in the retrieve API
        retrieve_url = TWEET_RETRIEVE_API.format(response.data['id'])
        response = self.hanyuan_client.get(retrieve_url)
        ...
        # test failed if more than 9 files uploaded
        files = [
            SimpleUploadedFile(
                name=f'selfie{i}.jpg',
    ...
    def test_pagination(self):
        ...
        for i in range(page_size * 2 - len(self.tweets1)):
            self.tweets1.append(self.create_tweet(self.hanyuan, 'tweet{}'.format(i)))

        tweets = self.tweets1[::-1]

        # pull the first page
        response = self.hanyuan_client.get(TWEET_LIST_API, {'user_id': self.hanyuan.id})
        ...
        response = self.hanyuan_client.get(TWEET_LIST_API, {
        ...
        new_tweet = self.create_tweet(self.hanyuan, 'a new tweet comes in')
